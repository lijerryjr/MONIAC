...
class Sand(object):
    ...
    def __init__(self, value, cx, cy, r, dx, dy, boing, gravity, velocity):
        self.value=value
        self.cx=cx
        self.cy=cy
        self.r=r
        ...
        #Set vector and magnitude
        
        self.magnitude=magnitude([dx, dy])
        self.vector=[dx/self.magnitude, dy/self.magnitude]
    ...
    def move(self):
        self.cx+=math.cos(math.radians(self.angle))*self.vector[0]*self.velocity
        self.cy+=math.sin(math.radians(self.angle))*self.vector[1]*self.velocity
    ...
    def updateVector(self, angle):
        self.dx=math.cos(angle)
        self.dy=math.sin(angle)
        self.magnitude=magnitude([self.dx, self.dy])
...
def init(data):
    ...
    data.sand=Sand(10, data.width/2, data.height/2, 10, data.dx, data.dy, 
                   data.boing, data.gravity, data.velocity)
...
def doStep(data):
    ...
    if data.sand.collides(data.wall):
        ...
        data.sand.angle=(data.wall.angle-math.pi+data.sand.angle)
        data.sand.angle%=(2*math.pi)
        #beta=2*data.sand.angle-2*data.wall.angle-math.pi
        print(data.wall.angle, data.sand.angle)
        print()
        data.sand.updateVector(data.sand.angle)
        data.sand.move()
    #Check boundaries
    if data.sand.cx-data.sand.r<=0:
        data.sand.cx=data.sand.r
        data.sand.dx=-data.sand.dx
        data.sand.angle=math.pi-data.sand.angle
        data.sand.angle%=2*math.pi
        data.sand.updateVector(data.sand.angle)
        data.sand.move()
    elif data.sand.cx+data.sand.r>=data.width:
        data.sand.cx=data.width-data.sand.r
        data.sand.dx=-data.sand.dx
        data.sand.angle=math.pi-data.sand.angle
        data.sand.angle%=2*math.pi
        data.sand.updateVector(data.sand.angle)
